...
module.exports = class Driver extends EventEmitter {
	...
	send(device, data, callback) {
		...
		if (frame.find(isNaN) || !dataCheck || dataCheck.constructor !== Object || !dataCheck.id) {
	...
	pair(socket) { // Pair sequence
		...
		socket.on('set_device', (data, callback) => {
			if (this.get(data)) {
				return callback(new Error('error.device_exists'));
			}
		...
		socket.on('done', (data, callback) => {
			if (!this.pairingDevice) {
				return callback(new Error('error.no_device'));
			}
