...
module.exports = class BaseDriver extends EventEmitter {
	constructor(Signal, driverConfig) {
		super();
		if (!driverConfig) {
	...
	sendCmd(device, cmd, callback, options) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!cmdKey) {
				const err = new Error(`Device of type ${device.cmdType} and subtype ${device.cmdSubType} does not have cmd ${cmd}`);
				this.logger.error(err);
				reject(err);
				return callback(err);
			}
	...
	generateDevice(data) {
		...
		return {
			...
			data: Object.assign({ overridden: false }, data, { driver_id: this.config.id }, typeObj),
		};
	}
	...
	getCmdLabel(cmdObj) {
		...
		if (!result) {
			const key = `ir_generator.cmds.${cmdObj.cmd}`;
			result = __(key);
			result = result === key ? null : result;
		}
	...
	getDevicesByCmd(cmdObj) {
		...
		for (const entry of this.devices) {
			if (cmdObj.type) {
				...
				if (entry[1].cmdType && entry[1].cmdType !== 'default') continue;
			}
			if (cmdObj.subType) {
				if (entry[1].cmdSubType !== cmdObj.subType) continue;
			} else {
				if (entry[1].cmdSubType && entry[1].cmdSubType !== 'default') continue;
			}
			result.push(entry[1]);
		}
		return result;
